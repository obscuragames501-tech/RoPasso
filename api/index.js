...
let db_matches = [
    { id: "match_1", home: "Ankaragücü", away: "Beşiktaş", stadium: "Eryaman Stadyumu", time: "20:00", date: "28.04.2026", status: "BİLET VAR" },
    { id: "match_2", home: "Ankaragücü", away: "Galatasaray", stadium: "Eryaman Stadyumu", time: "19:00", date: "02.05.2026", status: "YAKINDA" }
];

// --- ARAYÜZ ŞABLONU (DOKUNULMADI) ---
        ...
        <div class="nav-links">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    ${user && user.guilds ? user.guilds.map(g => `<a href="/server/${g.id}" class="dropdown-item"><b>#</b> ${g.name}</a>`).join('') : '<div class="dropdown-item">Giriş Yapın</div>'}
                </div>
            </div>
    ...
    res.send(ui(`
        ...
        <div style="display:grid; grid-template-columns: 2fr 1fr; gap:30px;">
            <div class="glass-card">
                ...
                <p>Profilin üzerinden biletlerini ve dijital kartlarını yönetebilirsin.</p>
                <div style="display:flex; gap:15px; margin-top:30px;">
                    <div style="background:#f8f8f8; padding:20px; border-radius:20px; flex:1; text-align:center;">
                        <h3 style="margin:0; font-size:28px;">${db_cards[req.session.user.id] ? 1 : 0}</h3>
                        <small style="color:#888;">Aktif Kart</small>
                    </div>
                    <div style="background:#f8f8f8; padding:20px; border-radius:20px; flex:1; text-align:center;">
                        <h3 style="margin:0; font-size:28px;">0</h3>
                        <small style="color:#888;">Maç Bileti</small>
                    </div>
                </div>
            </div>
            <div class="glass-card">
...
app.get('/matches', (req, res) => {
    ...
    let matchesHtml = db_matches.map(m => `
        <div class="match-row">
            <div class="team-info">
                <div style="text-align:right; flex:1;"><strong>${m.home}</strong></div>
                <div class="match-badge">VS</div>
                <div style="text-align:left; flex:1;"><strong>${m.away}</strong></div>
            </div>
            <div style="flex:1; text-align:center; color:#666; font-size:14px;">
                ${m.stadium}<br><b>${m.date} - ${m.time}</b>
            </div>
            <div style="flex:1; text-align:right;">
                ${m.status === 'BİLET VAR' 
                    ? `<a href="/buy/${m.id}" class="nav-btn" style="padding:8px 20px; font-size:12px;">BİLET AL</a>`
                    : `<span style="color:#aaa; font-weight:600;">SATIŞA KAPALI</span>`}
            </div>
        </div>
    `).join('');
    ...
    res.send(ui(`
        <div class="glass-card">
            <h2>Güncel Maçlar & Etkinlikler</h2>
            <p style="color:#888;">Stadyum girişlerinde sorun yaşamamak için biletinizi önceden rezerve edin.</p>
            <div style="margin-top:30px; border:1px solid #eee; border-radius:20px; overflow:hidden;">
                ${matchesHtml}
            </div>
        </div>
...
app.get('/api/auth/callback', async (req, res) => {
    ...
    try {
        ...
        req.session.user = { ...userRes.data, guilds: guildsRes.data.filter(g => (g.permissions & 0x8) === 0x8) };
...
app.get('/add-bot', (req, res) => {
    res.send(ui(`
        <div class="glass-card" style="text-align:center;">
            <img src="${ASSETS.BOT_BANNER}" style="width:100%; border-radius:20px;">
            <h1>Bot Kurulum Rehberi</h1>
